...
This resource implements the same features that are provided by the `website` configuration block of the [`aws_s3_bucket` resource](s3_bucket.html.markdown). To avoid conflicts or unexpected apply results, a lifecycle configuration is needed on the `aws_s3_bucket` to ignore changes to the internal `website` configuration block.  Failure to add the `lifecycle` configuration to the `aws_s3_bucket` will result in conflicting state results.
